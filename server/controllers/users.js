...
export const signup = async (req, res) => {
  ...
  try {
    ...
    const sql = `INSERT INTO users (name, username, profileImg, email, password) VALUES ('${name.replace(
      /[&\/\\#,+()$~%.'":*?<>{}]/g,
      ""
    )}','${username.replace(
      /[&\/\\#,+()$~%.'":*?<>{}]/g,
      ""
    )}','${profileImg}','${email.replace(
      /[&\/\\#,+()$~%.'":*?<>{}]/g,
      ""
    )}','${hashedPassword}')`;
    sqldb.query(sql, (err, user) => {
      if (err) throw err;
      galleryImg.forEach(
        (imageName, index) => (galleryImg[index] = [user.insertId, imageName])
      );
      const sql = "INSERT INTO galleryImg (user_id, image) VALUES ?";
      sqldb.query(sql, [galleryImg]);
      const token = jwt.sign({ email: email, id: user.insertId }, secret, {
        expiresIn: "1h",
      });
